...
@njit
def derivatives(y, delay_data, pSL, gamma_real, gamma_imag, kappa, phi_fb,
                interpol=False, dt=0.01, t=0.5):
    A, phi = y
    A_delay, phi_delay = delay_data

    if interpol:
        A_delayed = hermite_interpolation(A_delay[0], A_delay[1],
                                          A_delay[2], A_delay[3], dt, t)
        phi_delayed = hermite_interpolation(phi_delay[0], phi_delay[1],
                                            phi_delay[2], phi_delay[3], dt, t)
    else:
        A_delayed = A_delay[0]
        phi_delayed = phi_delay[0]

  
    dA_dt = pSL * A + gamma_real * A**3 + \
            kappa * A_delayed * np.cos(phi_fb + phi_delayed - phi)
    dphi_dt = 1.0 + gamma_imag * A**2 + \
              (kappa * A_delayed / A) * np.sin(phi_fb + phi_delayed - phi)

    return np.array([dA_dt, dphi_dt])

@njit
def rk4_step(y, delay_data, pSL, gamma_real, gamma_imag, kappa, phi_fb, dt):
    k1 = dt * derivatives(y, delay_data, pSL, gamma_real, gamma_imag,
                          kappa, phi_fb, False, dt)
    y_mid = y + 0.5 * k1
    k2 = dt * derivatives(y_mid, delay_data, pSL, gamma_real, gamma_imag,
                          kappa, phi_fb, True, dt, t=0.5)
    y_mid = y + 0.5 * k2
    k3 = dt * derivatives(y_mid, delay_data, pSL, gamma_real, gamma_imag,
                          kappa, phi_fb, True, dt, t=0.5)
    k4 = dt * derivatives(y + k3, delay_data, pSL, gamma_real, gamma_imag,
                          kappa, phi_fb, False, dt)
    return y + (k1 + 2 * k2 + 2 * k3 + k4) / 6

@njit
def solve(input_data, m, theta, N_v, pSL_base, eta, dt, tau,
    ...
    states[0] = [np.abs(np.random.rand() + 1j * np.random.rand()),
                 np.angle(np.random.rand() + 1j * np.random.rand())]
    ...
    for t in range(1, total_time_steps):
        ...
        delay_data = (
            np.array([states[delay_index, 0],
                      (states[delay_index, 0] - states[max(0, delay_index - 1), 0]) / dt,
                      states[max(0, delay_index - 1), 0],
                      (states[max(0, delay_index - 1), 0] - states[max(0, delay_index - 2), 0]) / dt]),
            np.array([states[delay_index, 1],
                      (states[delay_index, 1] - states[max(0, delay_index - 1), 1]) / dt,
                      states[max(0, delay_index - 1), 1],
                      (states[max(0, delay_index - 1), 1] - states[max(0, delay_index - 2), 1]) / dt])
        )

        
        states[t] = rk4_step(states[t - 1], delay_data, pSL,
                             gamma_real, gamma_imag, kappa, phi_fb, dt)
...
pd.DataFrame(S_with_bias).to_csv("state_matrix.csv", index=False)
print(pd.DataFrame(S_with_bias))

pd.DataFrame(input_data).to_csv("input_data.csv", index=False)
pd.DataFrame(x_component).to_csv("x_component.csv", index=False)
pd.DataFrame(z_component).to_csv("z_component.csv", index=False)
pd.DataFrame(pSL_values, columns=["pSL"]).to_csv("pSL_values.csv", index=False)

fig, axs = plt.subplots(2, 1, figsize=(12, 8), sharex=True)

axs[0].plot(time, J_t_values, color='b')
axs[0].set_title('Masked Input J(t)')
axs[0].set_ylabel('J(t)')
axs[0].grid(True)

axs[1].plot(time, X_abs, color='r')
axs[1].set_title('Reservoir Response')
axs[1].set_xlabel('Time')
axs[1].set_ylabel('X_abs')
axs[1].grid(True)
...
print(f"Max value of pSL: {np.max(pSL_values)}")
print(f"Min value of pSL: {np.min(pSL_values)}")
...
alpha_values = np.logspace(-10, 3, 100)
nmse_train_values = []
nmse_test_values = []
...
for alpha in alpha_values:
    ...
    nmse_train = calculate_nrmse(predictions_train, target_train)
    nmse_test = calculate_nrmse(predictions_test, target_test)
    nmse_train_values.append(nmse_train)
    nmse_test_values.append(nmse_test)

plt.figure(figsize=(10, 6))
plt.plot(alpha_values, nmse_train_values, marker='o', linestyle='-', color='b', lw=1.5, label='Training NRMSE')
plt.plot(alpha_values, nmse_test_values, marker='o', linestyle='-', color='r', lw=1.5, label='Testing NRMSE')
plt.xscale('log')
plt.xlabel('Lambda (Regularization Strength)')
plt.ylabel('Normalized Root Mean Squared Error (NRMSE)')
plt.title('NRMSE vs Lambda for Training and Testing Phases')
plt.legend()
plt.grid(True)
plt.show()


def nrmse_to_file(file_name, alpha, nrmse_train, nrmse_test):
    file_exists = os.path.isfile(file_name)
    
    with open(file_name, mode='a', newline='') as file:
        writer = csv.writer(file)
        if not file_exists:
            writer.writerow(['NRMSE Train', 'NRMSE Test'])
        writer.writerow([nrmse_train, nrmse_test])
...
print(f"NRMSE for alpha={alpha} on training set: {nrmse_train}")
print(f"NRMSE for alpha={alpha} on testing set: {nrmse_test}")


file_name = 'nrmse_results_x_to_x.csv'

nrmse_to_file(file_name, alpha, nrmse_train, nrmse_test)
